...
from speasy.core.cache import CacheCall, CacheItem, add_item, get_item, request_locker

from . import http
from .url_utils import extract_path, is_local_file
...
def _remote_open(url, timeout: int = http.DEFAULT_TIMEOUT, headers: dict = None, mode='rb'):
    ...
    if resp.status != 200:
        raise OSError(f"Could not open remote file {url}: HTTP {resp.status}")
    if 'b' in mode:
...
def any_loc_open(url, timeout: int = http.DEFAULT_TIMEOUT, headers: dict | None = None, mode='rb',
...
def list_files(url: str, file_regex: re.Pattern | str, disable_cache=False, force_refresh=False) -> list[str]:
