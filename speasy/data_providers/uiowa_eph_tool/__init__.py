...
def build_trajectories(origin: str, coordinate_system: str)-> Dict[str, SpeasyIndex]:
    ...
    for obs, code in __OBSERVERS_PER_ORIGINS_DICT__.get(origin, {}).items():
        meta = {
            'Id': code,
            'start_date': __TIME_RANGES__.get(obs, DateTimeRange(datetime(1958, 1, 1), datetime(2094, 1, 1))).start_time.isoformat(),
            'stop_date': __TIME_RANGES__.get(obs, DateTimeRange(datetime(1958, 1, 1), datetime(2094, 1, 1))).stop_time.isoformat(),
            'CoordinateSystem': coordinate_system,
            'Origin': origin,
            'Observer': obs
        }
        trajectories[fix_name(obs)] = ParameterIndex(name=obs, provider='UiowaEphTool', uid=_make_uid(origin, obs, coordinate_system), meta=meta)
    return trajectories
...
class UiowaEphTool(DataProvider):
    ...
    def __init__(self):
        DataProvider.__init__(self, provider_name='uiowaephtool', provider_alt_names=['UiowaEphTool'], min_proxy_version=Version("0.13.0"))
    ...
    @EnsureUTCDateTime()
    @ParameterRangeCheck()
    @Cacheable(prefix="UiowaEphTool_orbits", fragment_hours=lambda x: 24, version=version, entry_name=_make_cache_entry_name)
    ...
    def _get_orbit(self, product: str, start_time: datetime, stop_time: datetime,  extra_http_headers: Dict or None = None) -> Optional[SpeasyVariable]:
